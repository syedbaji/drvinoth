        ...
        var plugin = {
            ...
            loadImage: function (mediatype) {
                ...
                if(mediatype === 'youtube'){
                    img = $('<div class="iframe-holder d-flex align-items-center justify-content-center h-100 w-100"><iframe width="560" height="315" src="' + $(plugin.current).attr('href') + '" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>');
                    $('.lightbox__loading').remove();
                    $('.lightbox .iframe-holder').remove();
                    plugin.lightbox.append(img);
                    plugin.image = $("img", plugin.lightbox).hide();
                    plugin.resizeImage();
                }else{
                    img = $('<img src="' + $(plugin.current).attr('href') + '" draggable="false">');
            ...
            bindEvents: function () {
                $(plugin.items).click(function (e) {
                    ...
                    e.preventDefault();
                    plugin.current = self;
                    var mediatype = $(self).data('type');
                    plugin.loadImage(mediatype);
